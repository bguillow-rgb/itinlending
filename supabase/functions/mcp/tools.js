...
export function registerTools(server, logCall) {
    ...
    function countResults(payload) {
        if (!payload || typeof payload !== "object")
            return null;
        const p = payload;
        if (Number.isInteger(p.result_count))
    ...
    server.tool("search_guides", "Search 290+ editorial guides across the ITIN finance network (loans, mortgages, credit cards, credit scores — English and Spanish). Returns quick answers with canonical article URLs.", { query: z.string().min(2).max(120).describe("What the user wants to know, e.g. 'refinance car loan itin' or 'prestamo personal con itin'"), lang: LANG, site: z.enum(["lending", "creditcard", "creditscore"]).optional().describe("Restrict to one site in the network: lending, creditcard, or creditscore. Omit to search all three") }, ro("Search ITIN finance guides"), guarded("search_guides", async ({ query, lang, site }) => {
