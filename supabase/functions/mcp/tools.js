...
export function registerTools(server, logCall) {
    ...
    server.tool("search_guides", "Search 290+ editorial guides across the ITIN finance network (loans, mortgages, credit cards, credit scores — English and Spanish). Returns quick answers with canonical article URLs.", { query: z.string().max(120).min(2).max(200).describe("What the user wants to know, e.g. 'refinance car loan itin' or 'prestamo personal con itin'"), lang: LANG, site: z.enum(["lending", "creditcard", "creditscore"]).optional().describe("Restrict to one site in the network: lending, creditcard, or creditscore. Omit to search all three") }, ro("Search ITIN finance guides"), guarded("search_guides", async ({ query, lang, site }) => {
    ...
    server.tool("get_guide", "Get one guide by slug or approximate title: quick answer, FAQs, related guides, canonical URL.", { slug: z.string().max(200).min(2).max(120).describe("Guide slug, or an approximate title to match on"), lang: LANG }, ro("Get an ITIN finance guide"), guarded("get_guide", async ({ slug, lang }) => {
        const guides = await getGuides();
    ...
    server.tool("itin_state_info", "State-level context for ITIN holders: state/local taxes paid by undocumented immigrants (ITEP 2022), effective tax rate, driver's-license access law, and the state guide URL.", { state: z.string().max(120).min(2).max(30).describe("US state name or 2-letter code") }, ro("ITIN state facts"), guarded("itin_state_info", async ({ state }) => {
