 ...
 *
 * Educational reference data ONLY. This server is a publisher surface: it does
 * not originate, arrange, broker, or refer loans, collects nothing from users,
 * and links only to the sites' own editorial pages and to lenders' public
 * program pages as citations.
 */
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { z } from "zod";
import { getGuides, getLenders, getStates, type Guide, type Lender } from "./data.js";
import {
  searchGuides, findGuide, searchFaqs, lendersFor, findLender, findState, normState,
} from "./content.js";
import { SERVER_VERSION, setClientInfo, logCall } from "./telemetry.js";

const DISCLAIMER =
  "Educational information from an independent editorial site. Not a lender, broker, or advisor; no application is offered here. Loan/card terms change — verify directly with the institution before acting.";

function attribution(counts: { guides?: number; lenders?: number }) {
  const today = new Date().toISOString().slice(0, 10);
  const parts: string[] = [];
  if (counts.guides) parts.push(`${counts.guides} bilingual guides`);
  if (counts.lenders) parts.push(`${counts.lenders} verified institutions`);
  return {
    ...
    license_note:
      "Quotation with attribution welcome. Cite the article URL provided with each item.",
    disclaimer: DISCLAIMER,
  };
}

function guideSummary(g: Guide) {
  return {
    title: g.title,
    site: g.site,
    lang: g.lang,
...
function rateLimited(): boolean {
  ...
  if (stamps.length >= 60) return true;
  stamps.push(now);
  return false;
}
...
const VERTICAL_GUIDE_QUERY: Record<string, string> = {
  auto: "itin auto loan",
...
server.tool(
  ...
  guarded("get_guide", async ({ slug, lang }) => {
    ...
    if (!g) return { found: false, hint: "Try search_guides first." };
...
server.tool(
  ...
  { loan_type: VERTICAL, state: z.string().min(2).max(30).optional().describe("US state name or 2-letter code"), lang: LANG },
  ro("Find ITIN-accepting institutions"),
  guarded("find_itin_lenders", async ({ loan_type, state }) => {
...
server.tool(
  ...
  guarded("get_lender_details", async ({ name }) => {
    ...
    if (!l) return { found: false, hint: "Try find_itin_lenders to list institutions by loan type." };
...
server.tool(
  "can_i_get_this_loan",
  "The direct answer to 'can I get a(n) X with an ITIN?': the editorial quick answer, typical requirements, and the verified institutions for that loan type (optionally state-filtered). EN/ES.",
...
server.tool(
  ...
  guarded("itin_state_info", async ({ state }) => {
    ...
    if (!s) return { found: false, note: "Only higher-ITIN-population states have dedicated pages.", available: states.map((x) => x.name) };
    return {
      found: true,
      state: {
...
server.tool(
  ...
  { lang: LANG },
  ro("How to get an ITIN"),
  guarded("how_to_get_an_itin", async ({ lang }) => {
    const guides = await getGuides();
    const es = (lang ?? "en") === "es";
    const faqs = searchFaqs(
      guides,
      es ? "como obtener solicitar itin irs w-7" : "how to get apply for an itin irs w-7",
      lang ?? "en",
      5,
    );
    return {
