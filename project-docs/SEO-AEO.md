...
- **Sitemap** — auto-generated by `@astrojs/sitemap` each build; `404` and
  ...
  - **`lastmod` (fixed 2026-07-27, all 3 sites):** do NOT use a global
    `lastmod: new Date()` — it stamps every URL with build time, so the whole
    sitemap "changes" on each daily-content deploy and Google discounts it and
    stops re-reading (this cost us ~37 days of unread sitemap). Instead the
    `serialize()` hook sets a STABLE per-article `lastmod` from committed
    frontmatter (`updatedAt` || `publishedAt`) via a `buildArticleLastmodMap()`
    helper; static pages get no `lastmod`. Frontmatter is the only CI-safe date —
    file mtime and `git log` both churn under Actions' shallow checkout.
